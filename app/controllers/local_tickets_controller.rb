class LocalTicketsController < ApplicationController
  ...
  before_action :set_location_from_params
  
  # GET /printing_tickets
  # GET /printing_tickets.json
  def index
    
    @printing_tickets = PrintingTicket.where(location_id: @location_id).order_by(:project_id => 'desc').order_by(:name => 'desc')
#    session[:location_id] = @location_id
  ...
  def new
    ...
    @printing_ticket.location_id = @location_id  
  end
...
#    @printing_ticket.equipment_id = equipment
    ...
    respond_to do |format|
      @location_id = @printing_ticket.location_id
      if @printing_ticket.save
        format.html { redirect_to location_local_tickets_url(@location_id), notice: 'Printing ticket was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @printing_ticket.update(printing_ticket_params)
        format.html { redirect_to location_printing_tickets_url(@location_id), notice: 'Printing ticket was successfully updated.' }
        format.json { render :index, status: :ok, location: @printing_ticket }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to location_printing_tickets_url(@location_id) , notice: 'Printing ticket was successfully destroyed.' }
  ...
  private
    ...
    def printing_ticket_params
      params.require(:printing_ticket).permit(:name, :subname, :description, :width, :height, :qty, :single, :double, :material_id, :equipment_id, :location_id, :job_state_id, :material_idc, :location_id)
    end
